 ...
 * Updated with Persistent Defaulter Tracking
 * 
 * DEFAULTER LOGIC:
 * 1. Time Check: Only flag defaulters after September (month >= 9)
 * 2. Outstanding Amount: Must have amount_payable > 0
 * 3. Bill Existence: Must have at least one bill generated
 * 4. Persistent: Once flagged (after September), remains defaulter until full payment
 * 5. Multi-Year: Accounts with bills from previous years are marked as "Multi-Year"
 */
...
$sortBy = isset($_GET['sort_by']) ? $_GET['sort_by'] : 'amount_desc'; // amount_desc, amount_asc, name_asc

// Check if we should show defaulters based on September rule
$currentDate = new DateTime();
$currentMonth = (int)$currentDate->format('n'); // 1-12
$currentYear = (int)$currentDate->format('Y');

// Bills are prepared in September, so only flag defaulters after September (any year)
$showDefaulters = $currentMonth >= 9;

// For display purposes - show current year as billing context
...
try {
    ...
    if ($showDefaulters) {
        // Build WHERE clause for businesses - persistent defaulter tracking
        $businessWhereConditions = [
            "b.amount_payable > 0",
            "EXISTS (SELECT 1 FROM bills bl WHERE bl.reference_id = b.business_id AND bl.bill_type = 'Business')"
        ];
        $businessParams = [];
        ...
        if ($minAmount > 0) {
            $businessWhereConditions[] = "b.amount_payable >= ?";
            $businessParams[] = $minAmount;
        }
        
        $businessWhereClause = implode(' AND ', $businessWhereConditions);
        
        // Build WHERE clause for properties - persistent defaulter tracking
        $propertyWhereConditions = [
            "p.amount_payable > 0",
            "EXISTS (SELECT 1 FROM bills bl WHERE bl.reference_id = p.property_id AND bl.bill_type = 'Property')"
        ];
        $propertyParams = [];
        ...
        if ($minAmount > 0) {
            $propertyWhereConditions[] = "p.amount_payable >= ?";
        ...
        if ($selectedType === 'all' || $selectedType === 'business') {
            $businessQuery = "
                ...
                LEFT JOIN bills bl ON bl.reference_id = b.business_id AND bl.bill_type = 'Business'
                    AND bl.billing_year = (
                        SELECT MAX(billing_year) FROM bills 
                        WHERE reference_id = b.business_id AND bill_type = 'Business'
                    )
        ...
        if ($selectedType === 'all' || $selectedType === 'property') {
            $propertyQuery = "
                ...
                LEFT JOIN bills bl ON bl.reference_id = p.property_id AND bl.bill_type = 'Property'
                    AND bl.billing_year = (
                        SELECT MAX(billing_year) FROM bills 
                        WHERE reference_id = p.property_id AND bill_type = 'Property'
                    )
        ...
        // Combine defaulters (both arrays are guaranteed to be arrays now)
    ...
    switch ($sortBy) {
        case 'amount_asc':
            usort($allDefaulters, function($a, $b) {
                return $a['amount_payable'] <=> $b['amount_payable'];
            });
            break;
        case 'name_asc':
            usort($allDefaulters, function($a, $b) {
        ...
        default:
            usort($allDefaulters, function($a, $b) {
                return $b['amount_payable'] <=> $a['amount_payable'];
            });
            break;
    }
    ...
    $totalOutstanding = $totalDefaulters > 0 ? array_sum(array_column($allDefaulters, 'amount_payable')) : 0;
    ...
    foreach ($allDefaulters as $defaulter) {
        ...
        $zoneBreakdown[$zoneName]['total_amount'] += $defaulter['amount_payable'];
    }
    ...
    // Calculate next bill preparation date
    $nextBillDate = new DateTime();
    try {
        if ($currentMonth >= 9) {
            $nextBillDate->setDate($currentYear + 1, 9, 1);
        } else {
            $nextBillDate->setDate($currentYear, 9, 1);
        }
    } catch (Exception $dateException) {
        // Fallback to current date if date calculation fails
        $nextBillDate = new DateTime();
    }
    // Calculate additional statistics for persistent tracking
...
<body>
        ...
        <div class="main-content" id="mainContent">
            ...
            <div class="page-header">
                <div>
                    ...
                    <p class="page-subtitle">Persistent tracking of accounts with outstanding payments (post-September)</p>
                </div>
                <div class="page-actions">
                    ...
                    <?php if ($showDefaulters && $totalDefaulters > 0): ?>
            ...
            <div class="billing-status-card">
                <div class="billing-status-header">
                    ...
                    <div class="billing-status-info">
                        <h3>Persistent Defaulter Tracking</h3>
                        <p>Accounts remain flagged until full payment is made</p>
                    </div>
                </div>
                
                <div class="billing-details">
                    <div class="billing-detail">
                        <div class="billing-detail-label">Current Month</div>
                        <div class="billing-detail-value"><?php echo $currentDate->format('F Y'); ?></div>
                    </div>
                    <div class="billing-detail">
                        <div class="billing-detail-label">Billing Year</div>
                        <div class="billing-detail-value"><?php echo $billingYear; ?></div>
                    </div>
                    <div class="billing-detail">
                        <div class="billing-detail-label">Next Bill Preparation</div>
                        <div class="billing-detail-value"><?php echo $nextBillDate->format('F Y'); ?></div>
                    </div>
                    <div class="billing-detail">
                        ...
                        <div class="billing-detail-value">
                            <?php if ($showDefaulters && $totalDefaulters > 0): ?>
                                ...
                                <span style="color: #e53e3e;">⚠️ Persistent Active</span>
                            <?php else: ?>
                                <span style="color: #718096;">🕐 Pre-Bill Period</span>
                            <?php endif; ?>
                        </div>
                    </div>
                    <div class="billing-detail">
                        <div class="billing-detail-label">Logic</div>
                        <div class="billing-detail-value" style="font-size: 12px; line-height: 1.3;">
                            Defaulters remain flagged<br>
            ...
            <?php if (!$showDefaulters): ?>
            <!-- Pre-September Alert -->
            <div class="alert alert-info">
                <span class="icon-clock"></span>
                <strong>Pre-Bill Period:</strong> Bills have not been prepared yet for <?php echo $currentYear; ?>. 
                Persistent defaulter tracking will be active after September <?php echo $currentYear; ?>.
            </div>
            <?php elseif ($totalDefaulters > 0): ?>
            ...
            <div class="alert alert-warning">
                ...
                These accounts remain flagged until full payment is made.
            </div>
            <?php else: ?>
            <!-- No Defaulters Alert -->
            <div class="alert alert-success">
                <span class="icon-smile"></span>
                <strong>Excellent!</strong> All accounts have made full payments. No persistent defaulters found.
            </div>
            <?php endif; ?>
            ...
            <div class="stats-grid">
                <div class="stat-card danger">
                    ...
                    <div class="stat-subtitle">Persistent defaulter accounts</div>
                </div>
                ...
                <div class="stat-card info">
                    <div class="stat-header">
                        <div class="stat-title">Multi-Year Defaulters</div>
                        <div class="stat-icon">
                            <span class="icon-clock"></span>
                        </div>
                    </div>
                    <div class="stat-value"><?php echo number_format($multiYearDefaulters); ?></div>
                    <div class="stat-subtitle">From previous years</div>
                </div>
                ...
                <div class="stat-card success">
                    ...
                    <div class="stat-value"><?php echo number_format($currentYearDefaulters); ?></div>
                    <div class="stat-subtitle">From <?php echo $currentYear; ?></div>
                </div>
            </div>
                ...
                <div class="card-body">
                    ...
                    <div class="search-box">
                        ...
                        <input type="text" id="searchInput" class="search-input" 
                            placeholder="Search by name, account number, location, or defaulter type...">
                    </div>
                            ...
                            <tbody>
                                <?php foreach ($allDefaulters as $defaulter): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php 
                                            $amount = $defaulter['amount_payable'];
                                        ...
                                        <td>
                                            <?php 
                                            $defaulterType = $defaulter['defaulter_type'] ?? 'Current Year';
                                            $typeClass = $defaulterType === 'Multi-Year' ? 'badge-danger' : 'badge-warning';
                                            ?>
                                            <span class="badge <?php echo $typeClass; ?>">
                                                <?php echo htmlspecialchars($defaulterType); ?>
                                            </span>
                                        </td>
                                        <td>
                                            <?php if ($amount >= 1000): ?>
                                                <span class="badge badge-danger">Critical</span>
                                            <?php elseif ($amount >= 500): ?>
                    ...
                    <?php else: ?>
                        <div style="text-align: center; color: #718096; padding: 40px;">
                            ...
                            <p>All accounts have made full payments and are up to date.</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            <?php else: ?>
            <!-- Pre-September Message -->
            <div class="card">
                <div class="card-body" style="text-align: center; padding: 60px;">
                    <div style="font-size: 72px; margin-bottom: 20px; opacity: 0.3;">
                        <span class="icon-calendar"></span>
                    </div>
                    <h2 style="color: #2d3748; margin-bottom: 15px;">Bills Not Yet Prepared</h2>
                    <p style="color: #718096; font-size: 18px; margin-bottom: 20px;">
                        Bills for <?php echo $currentYear; ?> will be prepared in September. 
                        Persistent defaulter tracking will be available after bill preparation.
                    </p>
                    <div style="background: #f7fafc; padding: 20px; border-radius: 10px; display: inline-block;">
                        <strong>Next Bill Preparation:</strong> <?php echo $nextBillDate->format('F j, Y'); ?><br>
                        <small style="color: #718096;">Defaulters remain flagged until full payment is made</small>
                    </div>
                </div>
            </div>
            <?php endif; ?>
    ...
    <script>
        ...
        const showDefaulters = <?php echo json_encode($showDefaulters); ?>;

        // Initialize charts
        document.addEventListener('DOMContentLoaded', function() {
            if (showDefaulters && zoneLabels.length > 0) {
                initializeCharts();
            }
            if (showDefaulters) {
                initializeSearch();
