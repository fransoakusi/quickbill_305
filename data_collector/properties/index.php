...
try {
    ...
    $stats = $db->fetchRow("
        SELECT 
            COUNT(*) as total,
            SUM(CASE WHEN property_use = 'Residential' THEN 1 ELSE 0 END) as residential,
            SUM(CASE WHEN property_use = 'Commercial' THEN 1 ELSE 0 END) as commercial,
            SUM(CASE WHEN amount_payable > 0 THEN 1 ELSE 0 END) as defaulters,
            SUM(CASE WHEN created_by = ? THEN 1 ELSE 0 END) as my_properties
        FROM properties
    ", [$currentUser['user_id']]);
