<?php
/**
 * Officer - Businesses Index
 * List and manage businesses with search and filter capabilities
 */
...
try {
    ...
    $businessTypes = $db->fetchAll("SELECT DISTINCT business_type FROM businesses WHERE business_type != '' ORDER BY business_type");
    ...
    if (!empty($search)) {
        $whereConditions[] = "(b.business_name LIKE ? OR b.owner_name LIKE ? OR b.account_number LIKE ?)";
        $searchTerm = '%' . $search . '%';
        $params[] = $searchTerm;
        $params[] = $searchTerm;
        $params[] = $searchTerm;
    ...
    if (!empty($payment_status_filter)) {
        if ($payment_status_filter === 'Defaulter') {
            $whereConditions[] = "b.amount_payable > 0";
        } else {
            $whereConditions[] = "b.amount_payable <= 0";
        }
    }
    ...
    $sql = "SELECT b.*, z.zone_name, sz.sub_zone_name,
                   CASE WHEN b.amount_payable > 0 THEN 'Defaulter' ELSE 'Up to Date' END as payment_status,
    ...
    setFlashMessage('error', 'Error loading businesses: ' . $e->getMessage());
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Businesses - <?php echo APP_NAME; ?></title>
    ...
    <style>
        * {
        ...
        /* Original Styles from Businesses Index */
        .page-header {
            background: white;
            border-radius: 15px;
            padding: 30px;
        ...
        <div class="sidebar hidden" id="sidebar">
            <div class="sidebar-content">
                ...
                <div class="nav-section">
                    ...
                    <div class="nav-item">
                        <a href="add.php" class="nav-link <?php echo ($currentDir === 'businesses' && $currentPage === 'add.php') ? 'active' : ''; ?>">
                            <span class="nav-icon">
            ...
            <div class="page-header">
                <div class="d-flex justify-content-between align-items-start flex-wrap gap-3">
                    <div>
                        ...
                        <p class="page-subtitle">View and manage all registered businesses</p>
                    </div>
                    ...
                    <div class="filter-row">
                        <div class="form-group">
                            ...
                            <input type="text" 
                                   ...
                                   placeholder="Search business name, owner, account number...">
                        </div>
                    ...
                    <h5 class="table-title">
                        ...
                        <?php if ($totalRecords > 0): ?>
                            <span class="text-muted">
                                ...
                                <?php echo $totalRecords === 1 ? 'business' : 'businesses'; ?>)
                            </span>
                        <?php endif; ?>
                    </h5>
                ...
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <strong><?php echo htmlspecialchars($business['account_number']); ?></strong>
                                </td>
                                <td>
                                    <div>
                                    ...
                                    <small class="text-muted">
                                        Owner: <?php echo htmlspecialchars($business['owner_name']); ?>
                                    </small>
                                    <?php if (!empty($business['telephone'])): ?>
                                        <br><small class="text-muted">
                                            <i class="fas fa-phone"></i> 
                                            <?php echo htmlspecialchars($business['telephone']); ?>
                                        </small>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <div><?php echo htmlspecialchars($business['business_type']); ?></div>
                                    <small class="text-muted"><?php echo htmlspecialchars($business['category']); ?></small>
                                </td>
                                <td>
                                    <div><?php echo htmlspecialchars($business['zone_name'] ?? 'Not assigned'); ?></div>
                                    <?php if (!empty($business['sub_zone_name'])): ?>
                                        <small class="text-muted"><?php echo htmlspecialchars($business['sub_zone_name']); ?></small>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <strong class="<?php echo $business['amount_payable'] > 0 ? 'text-danger' : 'text-success'; ?>">
                                        <?php echo formatCurrency($business['amount_payable']); ?>
                                    </strong>
                                </td>
                                <td>
                                ...
                                <td>
                                    <span class="payment-badge payment-<?php echo strtolower(str_replace(' ', '', $business['payment_status'])); ?>">
                                        <?php echo htmlspecialchars($business['payment_status']); ?>
                                    </span>
                                </td>
                                <td>
                                    <small class="text-muted">
                                        <?php echo htmlspecialchars(trim($business['first_name'] . ' ' . $business['last_name'])); ?>
                                    </small>
                                </td>
                                <td>
                                    <div class="action-buttons">
                                        <a href="view.php?id=<?php echo $business['business_id']; ?>" 
                                           class="btn-action btn-view" 
                                           title="View Details">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="edit.php?id=<?php echo $business['business_id']; ?>" 
            ...
            <div class="pagination-container">
                ...
                <div class="text-center mt-3">
                    <small class="text-muted">
                        ...
                        (<?php echo number_format($totalRecords); ?> total businesses)
                    </small>
                </div>
            </div>
            <?php endif; ?>
